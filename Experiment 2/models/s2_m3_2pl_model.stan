functions {
  // Buerkner & Charpentier (2018, https://psyarxiv.com/9qkhj/)
...
transformed parameters {
  ...
  vector[K] b_sup_kk = b_sup + r_K[, 2];
}
model {
  vector[N] eta;
  for (n in 1:N)
    eta[n] = b_sup_kk[kk[n]] * mo(z_sup, (x_sup[n]-1));
  theta ~ std_normal();
  ...
  mu_beta ~ cauchy(0, 5);
  z_sup ~ dirichlet(rep_vector(1, 4));
  b_sup ~ cauchy(0, 5);
  y ~ bernoulli_logit(alpha[ii] .* (theta[jj] + beta[ii] + delta[kk] + eta));
}
