...
transformed parameters {
  ...
  vector[K] b_pol_kk = b_pol_for * x_for + b_pol_against * (1 - x_for) + r_K[, 2];
}
model {
  theta ~ std_normal();
  ...
  mu_beta ~ cauchy(0, 5);
  b_pol_for ~ cauchy(0, 5);
  b_pol_against ~ cauchy(0, 5);
  y ~ bernoulli_logit(alpha[ii] .* (theta[jj] + beta[ii] + delta[kk] + b_pol_kk[kk] .* x_pol));
}
