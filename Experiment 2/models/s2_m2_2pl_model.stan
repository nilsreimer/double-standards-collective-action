...
transformed parameters {
  ...
  vector[K] b_sjb_kk = b_sjb_for * x_for + b_sjb_against * (1 - x_for) + r_K[, 2];
}
model {
  theta ~ std_normal();
  ...
  mu_beta ~ cauchy(0, 5);
  b_sjb_for ~ cauchy(0, 5);
  b_sjb_against ~ cauchy(0, 5);
  y ~ bernoulli_logit(alpha[ii] .* (theta[jj] + beta[ii] + delta[kk] + b_sjb_kk[kk] .* x_sjb));
}
